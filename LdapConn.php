...
//    echo "<p>Ou: {$data->ou}</p>";
    ...
    $sql_sel_user = "SELECT * FROM profile WHERE Username = '".$data->username."'";
    $query_user = $conn->query($sql_sel_user);
       if(mysqli_num_rows($query_user)){
           $row = $query_user->fetch_array();
           $_SESSION['user_id'] = $row[0];
           $_SESSION['status'] = $row[7];
          
       }else {
           header( "Refresh:0; url=https://cad.kku.ac.th/systemnotice/register.php", true, 303);
